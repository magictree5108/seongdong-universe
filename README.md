...
**앱에서 쓰기**: 사이드바의 "온톨로지" 페이지 (`pages/1_🕸️_온톨로지.py`) —
① 질의응답(GraphRAG): 서브그래프를 컨텍스트로 근거 객체 ID·원문 URL을 단 답변
(그래프에 없으면 "확인되지 않는다"고 답함, 근거 충실성 표본 검증 6/6),
② 사업 360°: 사업 하나의 담당·근거조례·언급 보도·연도별 예산 한 화면,
③ 탐색기: 객체→링크 이웃으로 피벗하는 그래프 수사 흐름.
